...
<script>
    ...
    function filterEvents() {
            const filter = searchBar.value.toLowerCase();  //this is used for search bar
            const selectedType = typeFilter.value.toLowerCase();
            const selectedDate = dateFilter.value;

            cards.forEach(card => {
                const eventName = card.querySelector('.t')?.textContent.toLowerCase() || ''; //to search on different column in the table, add class to that specific column and replace it with this line's 'a'
                ...
                if (matchesSearch && matchesType && matchesDate) {
                    ...
                    card.style.display = 'none'; // Hide the row
                }
            });
        }
...
<script>
   ...
   // let timeout = null;
    ...
    function filterEvents() {
            const filter = searchBar.value.toLowerCase();  //this is used for search bar
            const filter2 = searchBar2.value.toLowerCase();  //this is used for search bar
            const selectedType = typeFilter.value.toLowerCase();
            const selectedDate = dateFilter.value;
