...
$eventId = isset($_GET['event_no']) ? (int)$_GET['event_no'] : 1;

// Fetch event data based on event ID where approvedstatus = 0 (unapproved events)
$sql = "SELECT name, long_dis FROM events WHERE no = ? AND approvedstatus = 0"; 
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $eventId); // Bind the event ID to the query
$stmt->execute();
...
if ($result->num_rows > 0) {
    ...
    // If no unapproved event is found, handle the error (optional)
    echo "No approved event found with the specified ID.";
}
